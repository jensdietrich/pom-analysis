...
public class MavenIndex {
    ...
    public static void main(String[] args) throws Exception {
        Indexer indexer = new DefaultIndexer(new DefaultSearchEngine(), new DefaultIndexerEngine(), new DefaultQueryCreator());
        IndexUpdater indexUpdater = new DefaultIndexUpdater(new DefaultIncrementalHandler(), new ArrayList<>());
        Map<String, IndexCreator> indexCreators = new HashMap<>();
        indexCreators.put("jarContent", new JarFileContentsIndexCreator());
        indexCreators.put("min", new MinimalArtifactInfoIndexCreator());
        indexCreators.put("maven-plugin", new MavenPluginArtifactInfoIndexCreator());
        MavenIndex m = new MavenIndex(indexer, indexUpdater, indexCreators);
        File indexDir = new File("maven-index");
        if (!indexDir.exists()) {
    ...
    public MavenIndex(Indexer indexer, IndexUpdater indexUpdater, Map<String, IndexCreator> indexCreators)
    {
        requireNonNull(indexer);
        requireNonNull(indexUpdater);
        requireNonNull(indexCreators);
    ...
    public void createOrUpdateIndex() throws IOException {
        File centralLocalCache = new File("stuff/central-cache");
        File centralIndexDir = new File("stuff/central-index");
    ...
    public void sampleSearches() throws IOException, InvalidVersionSpecificationException {
